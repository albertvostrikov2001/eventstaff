"""
Web-only deploy: syncs packages/web/src + web/public, rebuilds Web, restarts unity-web.
No API/DB changes. Credentials via .env.deploy.

Run: python deploy_web.py
"""
import paramiko
import tarfile
import io
...
REMOTE_TMP = '/tmp/unity_deploy_web.tar.gz'
...
def main():
    ...
    # Чистим прошлую распаковку, иначе tar не удаляет файлы, которых уже нет
    # в архиве (удалённые страницы «воскресают» при rsync). См. инцидент с
    # employer/payments и worker/earnings.
    run_cmd(ssh, 'rm -rf /opt/app/packages/web/src /opt/app/packages/web/public /opt/app/packages/shared/src')
