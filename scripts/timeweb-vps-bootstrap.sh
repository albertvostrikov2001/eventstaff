...
server {
    ...
    # Socket.IO WebSocket endpoint — must come before "location /"
    # Without this block nginx proxies /socket.io/ to Next.js which returns a 308 redirect.
    location /socket.io/ {
        proxy_pass http://127.0.0.1:${HOST_API_PORT}/socket.io/;
        proxy_http_version 1.1;
