...
export default function AdminMessagesLayoutClient({
  children,
}: {
  ...
  const isConversation =
    pathname.startsWith('/admin/messages/') && pathname !== '/admin/messages';
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 lg:h-[calc(100vh-5.5rem)] lg:flex-row lg:gap-0">
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col lg:overflow-y-auto">
          <ChatListPage hrefForRoom={(roomId) => `/admin/messages/${roomId}`} />
        </div>
      </aside>
      <main
