...
export default function WorkerConversationPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch(() => toast('Ошибка загрузки диалога', 'error'))
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast('Ошибка отправки сообщения', 'error');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] flex-col">
      <div className="flex items-center gap-3 border-b border-gray-200 bg-white px-4 py-3">
        <Link href="/worker/messages" className="rounded-full p-1.5 hover:bg-gray-100">
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-xs rounded-card px-4 py-2 text-sm lg:max-w-md ${
                    isOwn
