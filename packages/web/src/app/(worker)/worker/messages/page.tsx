...
interface Participant {
  id: string;
  workerProfile?: { firstName: string; lastName: string; photoUrl: string | null } | null;
  employerProfile?: { companyName: string | null; contactName: string | null; logoUrl: string | null } | null;
}
...
function getParticipantName(p: Participant): string {
  if (p.workerProfile) {
    return `${p.workerProfile.firstName} ${p.workerProfile.lastName}`.trim() || 'Специалист';
  }
  if (p.employerProfile) {
    return p.employerProfile.companyName ?? p.employerProfile.contactName ?? 'Работодатель';
  }
  return 'Пользователь';
}
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 rounded-card border border-gray-200 bg-white py-16 text-center">
            ...
            <p className="text-sm text-gray-500">Здесь появятся ваши диалоги с работодателями</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900">
                      {other ? getParticipantName(other) : 'Диалог'}
                    </p>
                    {lastMsg && (
                      <p className="truncate text-sm text-gray-500">{lastMsg.content}</p>
                    )}
                  </div>
                  {conv.lastMessageAt && (
