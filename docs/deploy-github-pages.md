...
- Динамические маршруты (`[id]`, `[roomId]` и т.д.) должны объявлять `generateStaticParams()`; для GitHub Pages у части страниц добавлены **заглушки** на время сборки (реальные id открываются на клиенте после загрузки SPA).
